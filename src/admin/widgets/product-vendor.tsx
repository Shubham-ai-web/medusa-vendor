...
import { useRemoveProductVendor } from "../hook/product";
...
const ProductBrandWidget = ({
    ...
    const { removeVendor, loading: removeLoading, error: removeError } = useRemoveProductVendor();
                ...
                {vendors.length > 0 ? (
                    vendors.map((vendor: any) => (
                        <li className="rounded-lg flex-row justify-between p-4 text-body-color dark:text-dark-6 flex text-sm bg-ui-bg-base hover:bg-ui-bg-base-hover">
                            ...
                            <Button variant="danger" size="small" title="Unlink Vendor"
                                onClick={() => handleUnlinkVendor(vendor.id)}
                                disabled={removingVendorId === vendor.id && removeLoading}
                                isLoading={removingVendorId === vendor.id && removeLoading}><Trash /></Button>
                        </li>
                    ))
                ) : (
                    <li className="rounded-lg text-center p-4 text-body-color dark:text-dark-6 text-sm bg-ui-bg-base hover:bg-ui-bg-base-hover">No
                        vendors assigned</li>
                )}
            </ul>
            <VendorProductDrawer
